...
13 Layout your map for print
14 Change your basemap
...
But there's a bit of a catch: If you look closely at our dataset, you'll see that there are columns labeled **Latitude** and **Longitude**. (You can call your columns anything you want; you'll tell QGIS which columns to use later.) As you may recall, computers use these coordinates to locate data precisely on a map. The process of moving from human-readable addresses to compter-readable coordinates is called **geocoding**.

**QGIS will not geocode your addresses for you**. This is in contrast to Tableau and some other programs, like ArcGIS, which *will*. Well, that's not *entirely* true; there are some QGIS plugins that will help you. But setting them up, while not incredibly difficult, requires a number of steps, such as requesting something called an API key, which authorizes you to use a geocoding service. I've concluded that the process of geocoding your data via QGIS (or via another service) is complex enough that including it in this tutorial would make the whole thing way too long. So we're working with data that is already geocoded.

If your data is not geocoded and you want to use QGIS, you can either use an external service to obtain geocoordinates and add them to your dataset in advance, or you can use one of QGIS's plugins. To obtain geocoordinates online, I've had the best luck with the batch geocoder from [LocalFocus](https://geocode.localfocus.nl/) and the [Google Sheets plugin from Awesome Table](https://workspace.google.com/marketplace/app/geocode_by_awesome_table/904124517349). To geocode via QGIS, you can [try the plugin MMGIS](https://www.gislounge.com/how-to-geocode-addresses-using-qgis/).
...
## 13 Layout your map for print
![Layout your map for print](steps-map_your_data_with_qgis/step-12.jpeg)
...
## 14 Change your basemap
![Change your basemap](steps-map_your_data_with_qgis/step-13.jpeg)
...
I unfortunately do not have time in this tutorial to walk you through the process of hosting your web-based QGIS map and embedding it in another site, but [these instructions](https://www.gispo.fi/en/blog/why-you-should-use-qgis2web-plugin-github-pages-to-produce-interactive-maps/) look useful. For information on how to embed your map in another website, see [here](https://github.com/taylorhixson/WF#integrating-with-a-website).
